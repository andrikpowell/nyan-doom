...
- A simple UV Plus difficulty can be added instead by including a `M_UVPLUS` graphic lump, which when detected will add a new sixth UV Plus skill (UV placement + SpawnMulti)
  - Disabled if `MAPINFO` features are detected
  - Disabled when trying to record demos (use `-skill 4 -coop_spawns` instead)
  - A custom confirm message can be added via a `NYANSKL6` text lump *(not MAPINFO formatted, just simple text without quotes or `\n`)*

**Random Music Option**
- Under "Nyan Options", there is a new option called "Play Random Music"
- Will randomly choose a track from the map lumps that exist
- Last 5 tracks will not be played if have been played already (this history is lost after quit)
- Savegames will retain the current "random" track
- Random music functionality is tied with IDMUSRR and "Random Music" hotkey. (Meaning if you trigger those, the random track will be remembered in the savegame)
