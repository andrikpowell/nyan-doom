...
**Single Custom Skill Support**
- Added the ability to define a sixth custom skill via the [NYANSKLL](../docs/nyanskll.md) lump
  - Single skill definition in the format of MAPINFO
  - Disabled if DSDAPREF `use_mapinfo` is detected
  - Disabled when demo recording/playback for compatibility reasons
- A more simple solution is provided via a "M_ULTRAP" graphic lump, which when detected will add a new UV Plus skill (UV placement + SpawnMulti) 
