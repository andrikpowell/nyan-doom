...
- Added the ability to clear and define custom skills via the [`SKILLDEF`](../docs/skilldef.md) lump
  - SKILLDEF is intended for non-UDMF maps, unlike the skill definition in MAPINFO.
  - Skill definitions in the format of MAPINFO
  - Disabled if DSDAPREF `use_mapinfo` is detected
- A simple UV Plus difficulty can be added instead by using a "M_UVPLUS" graphic lump, which when detected will add a new sixth UV Plus skill (UV placement + SpawnMulti)
  - This functionality is disabled upon detecting `SKILLDEF`
