...
- Removed autoload folders for REKKR (In the future TC games will not have an autoload folder by default)
- Disabled Dynamic Boom Help / Credit screens for TC games

**New Cheats / Tweaks**
- When typing TNTCOMP, SKILL, or IDCLEV, the current status will now be shown in a message (Woof)
- New SKILL cheat to change the skill level in-game (requires a new level to take effect) (Woof)
- TNTCOMP is now disabled in Heretic / Hexen / Doom v1.1
- Fixed IDMUS for Doom 1
- IDMUS cheat now tells you what map music you have chosen
- New IDNUT cheat
...
**Doom v1.1 Support (For Historical Purposes)**
- Nyan Doom can now play the 1.1 version of DOOM.WAD
- Autoloads v1.2 compatability (disabling TNTCOMP)
- Demo recording / playback is not supported for v1.1
- Certain curiousities with v1.1 support:
  - Added support for v1.1 two lump statusbar
  - Certain sfx are replaced that aren't present in v1.1 (D_GETPOW, D_ITMBK, D_DPIEHI, etc)
  - Rad suits count towards the item count
  - Nightmare skill disabled

**Weapon Alignment / Interpolation Fixes**
  - Fix Vanilla Weapon Switch Logic (example: dehacked chainsaw ammo weapon in RUST /// THE INFERNAL MACHINE)
...
**Other Stuff**
- You can now pick up multiple computer area maps (not in demos)
- "Clear Marks" is now "Clear Last Mark" (Woof)
- Fixed Dehacked Cheat Entry when recording demos
...
- Add SSG frame fix (from Woof)
- Even more improvements to the "Enhanced Lite Amp" option
- Animate code has been greatly simplified and improved
- Completely removed statusbar refresh logic (unnecessary with little performance impact)
- Fix Statusbar Face being wrong for a few tics when entering another level (Cripsy Doom)
