...
- Supports in wad `HEHACKED` lumps as well as the `-hhe` argument
...
- Add Crispy Heretic-like support for extra music names for Heretic (allow `MUS_E4M1` instead of the default `MUS_E1M6`)
...
- Add `Indiana` cheat back to Heretic
...
- Add Crispy Heretic-like support for Ultimate Doom E4 (allow `D_E4M1` instead of the default `D_E3M4`)
...
- Added better OpenGL player arrow
...
- Fix "Pain Palette Range" not being saved in config (thanks eshen for the bug report)
