...
static dboolean IN_KexIntermission(void)
{
  char name[9];
  ...
  if (gameepisode != 4 && gameepisode != 5)
    return false;
...
{
  return ((gameepisode > 0 && gameepisode < 4) || IN_KexIntermission()) &&
...
static void IN_DrawInterpic(void)
{
  ...
  // Draw KEX secret arrow on map
  if (IN_KexIntermission() && gameepisode == 5)
  {
    if (W_CheckNumForName("MAPE5_M9") != LUMP_NOT_FOUND)
...
void IN_DrawYAH(void)
{
    ...
    MN_DrTextA(s_HERETIC_IN_ENTERING, 160 - MN_TextAWidth(s_HERETIC_IN_ENTERING) / 2, 10);
    ...
    {                           // draw the destination 'X'
        ...
        if (IN_KexIntermission() && gameepisode == 5 && nextmap == 7)
...
void IN_DrawSingleStats(void)
{
    ...
    if (lf_author && gamemode == retail && !IN_HasInterpic())
    {
    ...
    MN_DrTextA(s_HERETIC_IN_FINISHED, x, yoffset);
    ...
    IN_DrTextB(s_HERETIC_IN_KILLS, 50, yoffset + 25);
    IN_DrTextB(s_HERETIC_IN_ITEMS, 50, yoffset + 50);
    IN_DrTextB(s_HERETIC_IN_SECRETS, 50, yoffset + 75);
    ...
    V_DrawShadowedNamePatch(237, yoffset + 25, "FONTB15", CR_DEFAULT, VPT_STRETCH);
    ...
    V_DrawShadowedNamePatch(237, yoffset + 50, "FONTB15", CR_DEFAULT, VPT_STRETCH);
    ...
    V_DrawShadowedNamePatch(237, yoffset + 75, "FONTB15", CR_DEFAULT, VPT_STRETCH);
    ...
    if (wbs->modified_partime)
    {
        IN_DrTextB(s_HERETIC_IN_TIME, 8, 120 + yoffset);
        IN_DrawTime(55, 120 + yoffset, hours, minutes, seconds);
        IN_DrTextB(s_HERETIC_IN_TOTAL, 8, 140 + yoffset);
    ...
    {
        IN_DrTextB(s_HERETIC_IN_TIME, 85, 120 + yoffset);
        IN_DrawTime(155, 120 + yoffset, hours, minutes, seconds);
        IN_DrTextB(s_HERETIC_IN_TOTAL, 85, 140 + yoffset);
