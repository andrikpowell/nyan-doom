...
//
// Animated Stbar Color Background
//

stbarcolor_t* stbar_lastframe;
stbarcolor_t* stbar_frames;
size_t stbar_maxframes;

int N_GetStbarAnimate(void)
{
    int i;
    int lumpnum = W_CheckNumForName(stbar);

    if (lumpnum == LUMP_NOT_FOUND)
        return false;

    for (i = 0; (size_t)i < n_maxanims; i++)
        if (n_anims[i].lump == lumpnum && n_anims[i].validcycle)
            return (AnimateTime / n_anims[i].ani_speed) % (n_anims[i].ani_end - n_anims[i].ani_start + 1);

    return false;    
}

static void N_ExtendStbarAnimateLimit(void)
{
    if (stbar_lastframe >= stbar_frames + stbar_maxframes)
    {
        size_t stbar_newmax = stbar_maxframes ? stbar_maxframes*2 : MAXANIMS;
        stbar_frames = Z_Realloc(stbar_frames, stbar_newmax*sizeof(*stbar_frames));
        stbar_lastframe = stbar_frames + stbar_maxframes;
        stbar_maxframes = stbar_newmax;
    }
}
...
int N_CheckStbarAnimateFrame(const char* lump)
{
    ...
    // Return 
    for (i = 0; (size_t)i < stbar_maxframes; i++)
        if (stbar_frames[i].lump == lumpnum)
            return true;
...
int N_GetStbarAnimateFrame(const char* lump, const char* col)
{
    ...
    for (i = 0; (size_t)i < stbar_maxframes; i++)
    {
        ...
        {
            if (!strcmp(col,"col"))
                return stbar_frames[i].col;
            else if (!strcmp(col,"col_top"))
                return stbar_frames[i].col_top;
        }
    }
...
void N_SetStbarAnimateFrame(const char* lump, byte col, byte col_top)
{
    ...
    N_ExtendStbarAnimateLimit();
    stbar_lastframe->lump = lumpnum;
    stbar_lastframe->col = col;
    stbar_lastframe->col_top = col_top;
    stbar_lastframe++;
}
