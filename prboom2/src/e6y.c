...
int HU_DrawDemoProgress(int force)
{
  ...
  if (dsda_IntConfig(dsda_config_playback_mouse_controls) && mouse_hide_timer > 0 && !timingdemo && !walkcamera.type)
  {
    ...
    V_FillRectTransMenu(0, bar_y, len, bar_h, playpal_lightest);
    if (len > 4)
      V_FillRectTransMenu(2, inner_y, len - 4, inner_h, playpal_darkest);
    ...
    for (int i = 0; i < playback_kf_size; i++)
    {
      ...
      V_FillRectTransMenu(x, inner_y, 1, inner_h, colrngs[CR_LIGHTBLUE][playpal_lightest]);
    }
    ...
    for (int i = 0; i < auto_kf_size; i++)
    {
      ...
      V_FillRectTransMenu(x, inner_y, 1, inner_h, colrngs[CR_GREEN][playpal_lightest]);
    }
    ...
    if (quick_kf.buffer)
    {
      ...
      V_FillRectTransMenu(x, inner_y, 1, inner_h, colrngs[CR_RED][playpal_lightest]);
    }

    V_FillRectTransMenu(len - 1, bar_y, 2, bar_h, playpal_lightest);

    return true;
  }
  else if (dsda_IntConfig(dsda_config_hudadd_demoprogressbar))
  {
    V_FillRectTransMenu(0, SCREENHEIGHT - 4, len - 0, 4, playpal_lightest);
    if (len > 4)
      V_FillRectTransMenu(2, SCREENHEIGHT - 3, len - 4, 2, playpal_darkest);
