...
static void ShowOverflowWarning(overrun_list_t overflow, int fatal, const char *params, ...)
{
  ...
  if (!overflows[overflow].promted)
  {
    ...
    snprintf(buffer, sizeof(buffer),
      ...
      name[overflow],
      "\nYou can change emulation behaviour for this overflow through in-game menu.",
      params);

    va_start(argptr, params);
    I_vWarning(buffer, overflows[overflow].warn, argptr);
    va_end(argptr);
  }
}
