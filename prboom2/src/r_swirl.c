...
static void R_InitDistortedFlats()
{
    ...
    offsets = Z_Realloc(offsets, SEQUENCE * FLATSIZE * sizeof(*offsets));
    offset = offsets;
...
byte *R_DistortedFlat(int flatnum, dboolean menus)
{
  static int swirltic = -1;
  static int swirlflat = -1;
  static byte distortedflat[FLATSIZE];
  int time = !menus ? leveltime : AnimateTime;
  ...
  if (swirltic != time)
  {
    if (!dsda_FrozenMode())
    {
      offset = offsets + ((time & (SEQUENCE - 1)) * FLATSIZE);
    }
    else
    {
      offset = offsets;
